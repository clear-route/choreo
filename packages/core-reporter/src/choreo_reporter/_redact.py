...
_STREAM_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"(authorization\s*[:=]\s*\w+\s+)([A-Za-z0-9._\-]+)", re.IGNORECASE),
...
# URL-with-credentials pattern handled separately because it has three
# groups (scheme, creds, suffix) — the two-group redaction logic can't
# rebuild the suffix. Covers amqp, redis, postgres, mysql, http, mongodb,
# and similar schemes.
...
def _redact_structured_inner(value: Any, stats: RedactionStats) -> Any:
    ...
    if isinstance(value, str):
        # Scrub URL credentials and token-shaped strings from leaf values
        # too — a payload field named `config` might still hold
        # `postgres://user:pw@host`, and the shape check on the key name
        # would miss it.
        return redact_stream(value, stats)
    return value
...
def redact_stream(text: str, stats: RedactionStats) -> str:
    ...
    out = _URL_CREDENTIALS.sub(_url_sub, out)
    return out
