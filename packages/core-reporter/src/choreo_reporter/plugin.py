...
`pip install choreo-reporter` is sufficient. Explicit opt-in via
`-p choreo_reporter.plugin` is also supported.
...
class _State:
    """Per-session reporter state. Stored on `config._choreo_reporter_state`."""
...
def pytest_addoption(parser: pytest.Parser) -> None:
    group = parser.getgroup("choreo-reporter", "Harness test report (PRD-007)")
    group.addoption(
        "--harness-report",
        action="store",
        dest="harness_report",
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    try:
        ...
        raise pytest.UsageError(f"[choreo-reporter] {e}") from e
    except Exception as e:
        warnings.warn(
            f"choreo-reporter: configure failed "
    ...
    config._choreo_reporter_state = state  # type: ignore[attr-defined]
...
def _get_state(config: pytest.Config) -> _State | None:
    return getattr(config, "_choreo_reporter_state", None)
...
def pytest_sessionstart(session: pytest.Session) -> None:
    ...
    try:
        ...
        warnings.warn(
            f"choreo-reporter: sessionstart failed ({type(e).__name__}: {e}); disabling for this run",
            RuntimeWarning,
            stacklevel=2,
        )
        state.disabled = True
...
def pytest_runtest_protocol(
    ...
    try:
        ...
        warnings.warn(
            f"choreo-reporter: metadata capture failed for {item.nodeid}: {e}",
            RuntimeWarning,
            stacklevel=2,
        )
...
def pytest_runtest_logreport(
    ...
    try:
        ...
        warnings.warn(
            f"choreo-reporter: logreport handler failed: {e}",
            RuntimeWarning,
            stacklevel=2,
        )
...
def pytest_sessionfinish(session: pytest.Session, exitstatus: int) -> None:
    ...
    try:
        ...
        warnings.warn(
            f"choreo-reporter: sessionfinish failed ({type(e).__name__}: {e}); report not written",
            RuntimeWarning,
            stacklevel=2,
        )
    finally:
...
def _write_report(session: pytest.Session, state: _State) -> None:
    ...
    if len(encoded.encode("utf-8")) > _HARD_FILE_CAP_BYTES:
        warnings.warn(
            f"choreo-reporter: report exceeds {_HARD_FILE_CAP_BYTES} bytes; "
    ...
    try:
        ...
        warnings.warn(
            f"choreo-reporter: HTML render failed "
...
def _harness_version() -> str:
    try:
        ...
        return getattr(choreo, "__version__", "unknown")
    except Exception:
        return "unknown"
