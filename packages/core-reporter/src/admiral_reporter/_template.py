...
.harness-report .hr-toolbar label:has(input[type="search"]) { display: inline-flex; align-items: center; gap: var(--hr-space-2); }
.harness-report .hr-search-count { font-size: var(--hr-text-xs); color: var(--hr-text-muted); font-variant-numeric: tabular-nums; font-family: var(--hr-font-mono); min-width: 0; }
.harness-report .hr-search-count:empty { display: none; }
.harness-report .hr-search-count[data-empty="true"] { color: var(--hr-fail); }
...
.harness-report .hr-file-header:hover .hr-caret { color: var(--hr-text); background: var(--hr-bg); border-color: var(--hr-border-strong); }
.harness-report .hr-caret { display: inline-flex; width: 22px; height: 22px; align-items: center; justify-content: center; color: var(--hr-text); font-size: 14px; font-weight: 700; background: var(--hr-surface-2); border: 1px solid var(--hr-border-strong); border-radius: var(--hr-radius-sm); transition: transform var(--hr-dur-medium) var(--hr-ease), color var(--hr-dur-fast) var(--hr-ease), background-color var(--hr-dur-fast) var(--hr-ease), border-color var(--hr-dur-fast) var(--hr-ease); flex-shrink: 0; line-height: 1; }
...
/* Scenario group — uniform styling whether a test has one scenario
 * or many. Each scenario renders as a thin "scenario NAME" section
 * header above a flowing body; multiple scenarios stack as siblings
 * separated by the header's bottom border. Card chrome (border,
 * background tint, extra padding) would make multi-scenario detail
 * panes feel denser than single-scenario ones for no informational
 * gain — the body content (handles, timeline, replies) is what the
 * reader is here for. */
.harness-report .hr-scenario { margin-bottom: var(--hr-space-4); }
.harness-report .hr-scenario-header { padding: var(--hr-space-2) 0 var(--hr-space-3) 0; display: flex; gap: var(--hr-space-3); align-items: center; cursor: pointer; border-bottom: 1px solid var(--hr-border); margin-bottom: var(--hr-space-3); transition: background-color var(--hr-dur-fast) var(--hr-ease); }
.harness-report .hr-scenario-header:hover { background: transparent; }
.harness-report .hr-scenario-header:hover > .hr-caret { background: var(--hr-bg); border-color: var(--hr-border-strong); }
.harness-report .hr-scenario[aria-expanded="false"] > .hr-scenario-header > .hr-caret { transform: rotate(-90deg); }
.harness-report .hr-scenario[aria-expanded="false"] .hr-scenario-body { display: none; }
.harness-report .hr-scenario-name { font-weight: 500; color: var(--hr-text-muted); font-size: var(--hr-text-sm); font-family: var(--hr-font-mono); }
...
.harness-report .hr-handle-header:hover .hr-caret { color: var(--hr-text); background: var(--hr-bg); border-color: var(--hr-border-strong); }
...
.harness-report .hr-handle .hr-caret { flex-shrink: 0; }
...
(function () {
  ...
  function buildTestRow(test, state) {
    ...
    if (test.admiral_meta && test.admiral_meta.tags) {
      test.admiral_meta.tags.forEach(function (tag) {
  ...
  function renderDetail(state) {
    ...
    if (test.admiral_meta) {
      var metaLine = el('div', { 'class': 'hr-scenario-meta' });
      if (test.admiral_meta.timeout_ms) {
        metaLine.appendChild(el('span', {}, 'budget ' + test.admiral_meta.timeout_ms + ' ms'));
      }
      if (test.admiral_meta.tags) {
        test.admiral_meta.tags.forEach(function (tag) {
  ...
  function apply(state) {
    ...
    announce(visible);
    updateSearchCount(state, visible);
    renderDetail(state);
  }

  // Show a live match count next to the search input so users know
  // whether an empty tree is "no matches" or "everything's collapsed".
  // The count reflects all active filters, not just the search term —
  // we only surface it when the user is actively searching, which is
  // when "N results" is most useful.
  function updateSearchCount(state, visible) {
    var node = document.querySelector('[data-field="search-count"]');
    if (!node) { return; }
    if (!state.search) {
      node.textContent = '';
    ...
    node.textContent = visible + ' match' + (visible === 1 ? '' : 'es');
    if (visible === 0) {
...
_HTML_TEMPLATE = (
    ...
    '    <label>search: <input type="search" data-filter="search" aria-label="search tests"><span class="hr-search-count" data-field="search-count" aria-hidden="true"></span></label>\n'
