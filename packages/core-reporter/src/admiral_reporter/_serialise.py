...
* Wire ids land at the report boundary via
  `admiral.redaction.redact_correlation_id` (single source of truth — the
    ...
    if isinstance(state, StageReplyState):
        if state is StageReplyState.ARMED:
            raise AssertionError(
                ...
                "freeze-path bug in admiral.stage._StageScenarioScope"
            )
...
_LOG = logging.getLogger("admiral_reporter.serialise")
...
def _serialise_stage_scenario(
    ...
    *,
    ...
    # hash-redacted via admiral.redaction.redact_correlation_id. Sourced
