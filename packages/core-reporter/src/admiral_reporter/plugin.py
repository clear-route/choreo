...
`pip install admiral-reporter` is sufficient. Explicit opt-in via
`-p admiral_reporter.plugin` is also supported.
...
import pytest
from admiral._reporting import (
    current_test_nodeid,
    register_observer,
    unregister_observer,
)
...
class _State:
    """Per-session reporter state. Stored on `config._admiral_reporter_state`."""
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    try:
        ...
        raise pytest.UsageError(f"[admiral-reporter] {e}") from e
    except Exception as e:
        warnings.warn(
            f"admiral-reporter: configure failed "
    ...
    config._admiral_reporter_state = state  # type: ignore[attr-defined]
...
def _build_state(config: pytest.Config) -> _State:
    ...
    stream_redact = not bool(config.getoption("harness_report_no_stream_redact", False))
...
def _get_state(config: pytest.Config) -> _State | None:
    return getattr(config, "_admiral_reporter_state", None)
...
def pytest_sessionstart(session: pytest.Session) -> None:
    ...
    try:
        ...
        warnings.warn(
            f"admiral-reporter: sessionstart failed ({type(e).__name__}: {e}); disabling for this run",
            RuntimeWarning,
            stacklevel=2,
        )
        state.disabled = True
...
def pytest_runtest_protocol(
    ...
    try:
        ...
        warnings.warn(
            f"admiral-reporter: metadata capture failed for {item.nodeid}: {e}",
            RuntimeWarning,
            stacklevel=2,
        )
...
def _note_item_metadata(state: _State, item: pytest.Item) -> None:
    ...
    # Extract @pytest.mark.admiral(...) metadata if present.
    admiral_meta: dict[str, object] = {}
    admiral_marker = item.get_closest_marker("admiral")
    if admiral_marker is not None:
        if admiral_marker.kwargs.get("timeout_ms") is not None:
            admiral_meta["timeout_ms"] = admiral_marker.kwargs["timeout_ms"]
        if admiral_marker.kwargs.get("tags"):
            admiral_meta["tags"] = list(admiral_marker.kwargs["tags"])
...
def pytest_runtest_logreport(
    ...
    try:
        ...
        warnings.warn(
            f"admiral-reporter: logreport handler failed: {e}",
            RuntimeWarning,
            stacklevel=2,
        )
...
def pytest_sessionfinish(session: pytest.Session, exitstatus: int) -> None:
    ...
    try:
        ...
        warnings.warn(
            f"admiral-reporter: sessionfinish failed ({type(e).__name__}: {e}); report not written",
            RuntimeWarning,
            stacklevel=2,
        )
    finally:
...
def _write_report(session: pytest.Session, state: _State) -> None:
    ...
    final_duration_ms = (time.monotonic() - state.collector.started_mono) * 1000
    ...
    if expected_workers:
        merge = merge_partials(state.report_dir, expected_workers=expected_workers)
        # Under xdist the master receives `pytest_runtest_logreport` events
        # forwarded from every worker, so its own `collector.tests` holds a
        # shadow copy of every test — but without file metadata
        # (`pytest_runtest_protocol` only runs on the worker that owns the
        # item) and without scenarios (observer callbacks fire in the
        # worker process, not the master). Extending `payload["tests"]`
        # therefore duplicates every test: one empty master record plus
        # one populated worker record. The worker partials are the
        # authoritative source, so replace rather than extend.
        payload["tests"] = list(merge.merged_tests)
        ...
        if _stage_transports:
            ...
            from admiral.redaction import REDACTION_VERSION as _RV
    ...
    try:
        ...
        warnings.warn(
            f"admiral-reporter: HTML render failed "
...
def _harness_version() -> str:
    try:
        ...
        return getattr(admiral, "__version__", "unknown")
    except Exception:
        return "unknown"
...
def _xdist_info(session: pytest.Session, state: _State) -> dict[str, Any] | None:
    num = getattr(session.config.option, "numprocesses", None)
    if state.is_worker:
        return {
