...
SENTINEL_CONTENT = "core-reporter v1 output directory\n"
...
def validate_report_path(path: Path) -> Path:
    ...
    if _is_within_home(resolved):
        raise UnsafeReportPath(
            f"refusing to write to user home directory {resolved!s}"
        )

    for vcs in (".git", ".svn", ".hg"):
        if (resolved / vcs).exists():
            raise UnsafeReportPath(
                f"refusing to write to a VCS root: {resolved / vcs!s} exists"
            )

    parent = resolved.parent
    if not parent.exists():
        raise UnsafeReportPath(
            f"report path parent does not exist: {parent!s}"
        )
    ...
    try:
        ...
        raise UnsafeReportPath(
            f"cannot stat report path parent {parent!s}: {e}"
        ) from e
...
def prepare_output_dir(report_path: Path) -> Path:
    ...
    if safe.exists():
        if not safe.is_dir():
            raise UnsafeReportPath(
                f"{safe!s} exists and is not a directory"
            )
...
def atomic_write_text(report_path: Path, filename: str, content: str) -> Path:
    ...
    if filename not in KNOWN_FILENAMES:
        raise ValueError(
            f"atomic_write_text may only emit known files; got {filename!r}"
        )
