"""core-reporter — pytest plugin that emits a test-report directory at the end
of every pytest run (PRD-007).

This package is a consumer-facing sibling of `core`; `core` itself has no
dependency on pytest, and this package contributes the reporting pipeline
through the `core._reporting` observer seam plus standard pytest hooks.
...
try:
    __version__ = _pkg_version("core-reporter")
