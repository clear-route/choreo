...
def write_partial(
    report_dir: Path, worker_id: str, payload: dict[str, Any]
) -> None:
...
def merge_partials(
    report_dir: Path, expected_workers: list[str]
) -> MergeOutcome:
    ...
    for entry in sorted(partial_dir.iterdir()):
        if not entry.name.startswith("worker-") or not entry.name.endswith(
            ".json"
        ):
            continue
