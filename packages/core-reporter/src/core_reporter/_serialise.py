...
from core._redact import redact_matcher_description
from core.matchers import MatchFailure
...
from ._redact import RedactionStats, redact_stream, redact_structured
...
def derive_test_outcome(
    pytest_outcome: str, scenario_outcomes: list[str]
) -> str:
...
def serialise_match_failure(
    failure: MatchFailure, stats: RedactionStats
) -> dict[str, Any]:
    ...
    if failure.children:
        out["children"] = [
            serialise_match_failure(c, stats) for c in failure.children
        ]
    return out
...
def serialise_reply_report(report: ReplyReport) -> dict[str, Any]:
    ...
    return {
        ...
        "matcher_description": redact_matcher_description(
            report.matcher_description
        ),
