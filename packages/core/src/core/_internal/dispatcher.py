...
class SurpriseEntry:
    ...
    topic: str
    correlation_id: str
    classification: str  # "unknown_scope" | "timeout_race" | "no_correlation_field" | "no_extractor"
    size: int


# Modules whose loaders execute payload-supplied code — forbidden as extractors.
_FORBIDDEN_EXTRACTOR_MODULES: frozenset[str] = frozenset(
...
class Dispatcher:
    ...
    def dispatch(
        ...
        *,
        ...
        classification = (
            "timeout_race" if correlation_id in self._deregistered else "unknown_scope"
        )
        self._surprise_log.append(
            SurpriseEntry(
                topic=topic,
