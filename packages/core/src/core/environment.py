...
For an LBM-only deployment the allowlist might have `lbm_resolvers`. For a
Kafka deployment it might have `kafka_brokers` and `client_ids`. For NATS,
`nats_servers`. The library does not prescribe which. Transports validate
their own configured values against the categories they care about;
unknown categories in the file are ignored silently (unused, not rejected)
so one file can cover several transports.
"""
...
class EnvironmentError(Exception):
    """Base for allowlist-guard failures.

    The name is retained from the original ADR-0006 shape; `AllowlistError`
    would read more clearly, but renaming base exceptions silently breaks
    callers. Subclasses carry the useful information."""


class HostNotInAllowlist(EnvironmentError):
    """A host (LBM resolver, NATS server, Kafka broker, …) is not on the allowlist."""


class AllowlistConfigError(EnvironmentError):
...
class Allowlist:
    ...
    def enforce(
        ...
        *,
        ...
        (e.g. ``"NATS server"``, ``"LBM resolver"``). `normalise`, when given,
        ...
        for value in values:
            ...
            raise HostNotInAllowlist(
                f"{label} {value!r} is not on the allowlist at "
                f"{self.source_path}"
            )
...
def load_allowlist(path: Path) -> Allowlist:
    ...
    for key, value in raw.items():
        if not isinstance(value, list):
            raise AllowlistConfigError(
                f"{path}: value for {key!r} must be a list, got "
