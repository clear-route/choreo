...
Installation:
    pip install 'admiral[redis]'   # pulls redis >= 5.0
"""
...
class RedisTransport:
    ...
    def __init__(
        ...
        *,
        ...
        self._pubsub_lock: asyncio.Lock | None = None
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            raise TransportError(
                "RedisTransport requires redis — install with `pip install 'admiral[redis]'`"
            ) from e
        ...
        try:
            ...
            try:
                ...
                pass
            raise TransportError(
                f"could not connect to Redis at {safe_url(self._url)!r}: {e}"
            ) from e
    ...
    def subscribe(self, topic: str, callback: TransportCallback) -> None:
        if self._pubsub is None:
            raise RuntimeError("RedisTransport is not connected; cannot subscribe")
    ...
    def publish(
        ...
        *,
        ...
        if self._client is None:
            raise RuntimeError("RedisTransport is not connected; cannot publish")
        loop = asyncio.get_running_loop()
        client = self._client
