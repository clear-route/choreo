...
Routing model:
    ...
    ``admiral``). Every ``publish(topic, ...)`` call publishes to that
...
Installation:
    pip install 'admiral[rabbitmq]'   # pulls aio-pika >= 9.0
"""
...
class _Sub:
    ...
    ready: asyncio.Future[tuple[Any, Any]] = field(default_factory=asyncio.Future)
...
class RabbitTransport:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            raise TransportError(
                "RabbitTransport requires aio-pika — install with `pip install 'admiral[rabbitmq]'`"
            ) from e
        ...
        except (TimeoutError, AMQPConnectionError, OSError) as e:
            raise TransportError(
                f"could not connect to RabbitMQ at {safe_url(self._url)!r}: {e}"
            ) from e
    ...
    def subscribe(self, topic: str, callback: TransportCallback) -> None:
        if self._exchange is None:
            raise RuntimeError("RabbitTransport is not connected; cannot subscribe")
        loop = asyncio.get_running_loop()
        sub = _Sub(callback=callback, task=None)  # type: ignore[arg-type]

        async def _do_subscribe() -> None:
            try:
                queue_name = f"admiral.{uuid.uuid4().hex[:12]}"
    ...
    def publish(
        ...
        *,
        ...
        if self._exchange is None:
            raise RuntimeError("RabbitTransport is not connected; cannot publish")

        try:
            import aio_pika
        except ImportError as e:
            raise TransportError("RabbitTransport requires aio-pika") from e
