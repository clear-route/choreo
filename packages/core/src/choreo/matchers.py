...
def _normalise_path(path: Path) -> tuple[PathPart, ...]:
    ...
    for p in parts:
        if not isinstance(p, (str, int)):
            raise TypeError(f"path parts must be str or int; got {type(p).__name__} in {path!r}")
    return parts
...
class _Predicate:
    ...
    def match(self, payload: Any) -> MatchResult:
        ...
        if ok:
            msg = (
                f"{path_label} {sym} {self.op_value!r}".lstrip()
                if self.parts
...
def payload_contains(substring: bytes) -> Matcher:
    return _PayloadContains(substring=substring, description=f"contains {substring!r}")
    ...
    def match(self, payload: Any) -> MatchResult:
        ...
        for m in self.matchers:
            ...
            child_failures.append(
                r.failure or MatchFailure("predicate", ROOT_PATH, m.description, None)
            )
        return MatchResult(
            False,
...
class _AnyElement:
    ...
    def match(self, payload: Any) -> MatchResult:
        ...
        for i, item in enumerate(payload):
            ...
            child = r.failure or MatchFailure("predicate", ROOT_PATH, self.inner.description, item)
...
def _run_plan(
    ...
    if node.kind == _PLAN_MATCHER:
        ...
        if result.matched:
            return None
        child = result.failure or MatchFailure("predicate", path_str, matcher.description, payload)
    ...
    if node.kind == _PLAN_DICT:
        if not isinstance(payload, dict):
            return (
                ...
                MatchFailure("type_mismatch", path_str, {"type": "dict"}, type(payload).__name__),
            )
    ...
    if node.kind == _PLAN_LIST:
        if not isinstance(payload, list):
            return (
                ...
                MatchFailure("type_mismatch", path_str, {"type": "list"}, type(payload).__name__),
            )
