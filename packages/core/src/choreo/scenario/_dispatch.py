...
_LOG = logging.getLogger("choreo.scenario")
...
def _register_expectation(context: _ScenarioContext, topic: str, matcher: Matcher) -> Handle:
    from choreo.matchers import _expected_shape
    ...
    def on_message(msg_topic: str, raw_payload: bytes) -> None:
        ...
        # Correlation filter (ADR-0019). When the policy extracts no id
        # (NoCorrelationPolicy, or a message without the correlation field),
        # the filter is skipped and the matcher sees every routed message —
        # broadcast fallback. When the policy extracts an id that differs
        # from the scope's, the message is for another scope; drop it.
...
def _register_reply(
    ...
    *,
    ...
    def on_trigger(msg_topic: str, raw_payload: bytes) -> None:
        ...
        # Correlation filter — identical to `expect` (ADR-0018, ADR-0019).
        if scope_corr is not None:
            try:
                msg_corr = policy.read(Envelope(topic=msg_topic, payload=payload))
            except Exception as e:
        ...
        # Correlation stamping + override detection (ADR-0018, ADR-0019).
        # The policy's write() may refuse an explicit override that fails
        # its namespace rule (e.g. DictFieldPolicy with a prefix rejects
        # overrides that do not start with the prefix). Policy exceptions
        # other than CorrelationIdNotInNamespaceError surface as a wrapped
        # CorrelationPolicyError and fail the reply with the policy class
        # name.
        if scope_corr is not None:
            try:
                ...
                return
            # Detect override: read back what the policy ended up with; if
            # it differs from scope_corr, the builder supplied its own id.
            try:
...
async def _await_all(context: _ScenarioContext, *, timeout_ms: int) -> ScenarioResult:
    ...
    # Seal the timeline BEFORE snapshotting so any in-flight subscriber
    # callback that fires between snapshot and `__aexit__`'s unsubscribe
    # loop sees `sealed=True` and becomes a silent no-op. Without this,
    # a late inbound mutates `dropped`/`record_errors` post-snapshot,
    # producing a consumer-visible inconsistency between
    # `len(result.timeline)` and `result.timeline_dropped`. Stage's
    # `_do_await_all` does the same; this single-`Harness` site was the
    # gap.
