...
def test_bareword_with_embedded_equals_should_redact_as_a_whole() -> None:
    ...
    assert (
        redact_matcher_description("account=secret=with=equals")
        == "account=<value>"
    )
...
def test_values_containing_escaped_quotes_should_redact_as_a_whole() -> None:
    ...
    assert (
        redact_matcher_description('note="\\"inner\\""')
        == "note=<value>"
    )
...
def test_nested_matcher_composition_should_redact_every_literal() -> None:
    ...
    assert (
        redact_matcher_description(
            "all_of(field_equals(account='ACME-1'), field_equals(qty=100))"
        )
