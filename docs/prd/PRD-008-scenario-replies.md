...
- **Wire [PRD-002](PRD-002-scenario-dsl.md)'s `with_correlation(field_name)`** through the existing `expect` filter and `Scenario.publish` injection at [packages/core/src/core/scenario.py:447](../../packages/core/src/core/scenario.py#L447) and [:512-521](../../packages/core/src/core/scenario.py#L512-L521). The field name must reach `_Reply` dispatch and reply-stamping without a second hardcoded path. Per [ADR-0018](../adr/0018-reply-correlation-scoping.md) §Timeline, shipping with `"correlation_id"` hardcoded would make the primitive unusable for consumers whose correlation field has a different name.
